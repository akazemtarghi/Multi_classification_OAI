...
def SplittingData (root='C:/Users/Amir Kazemtarghi/Documents/INTERNSHIP/data/IDnKL.csv',
    ...
    """
    csv_file = pd.read_csv(root)
    file1 = csv_file.copy()
    file1.drop_duplicates(subset=['ID'], inplace=True)
    file1 = file1.reset_index(drop=True)
    file2 = file1['ID']
    train_indices, test_indices = train_test_split(file2, test_size=testsize)
    train_split = csv_file.loc[csv_file['ID'].isin(train_indices)]
    train_split = train_split.reset_index(drop=True)
    train_split = train_split.drop(columns=['Unnamed: 0'])
    test_split = csv_file.loc[csv_file['ID'].isin(test_indices)]
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, epoch, nfold):
        nfold = str(nfold)
        epoch = str(epoch)
        score = -val_loss
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(val_loss, model, epoch, nfold)
    ...
    def save_checkpoint(self, val_loss, model, epoch, nfold):
        '''Saves model when validation loss decrease.'''
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        filename = 'checkpoint' + '_fold_' + nfold +'.pt'
...
def Training_dataset(data_loaders, model, patience, n_epochs, namefold, nfold, scheduler):
    ...
    for epoch in range(n_epochs):
        ...
        early_stopping(valid_loss, model, epoch, nfold)
    ...
    checkpoint_name = 'checkpoint' + '_fold_' + str(nfold) + '.pt'
...
train_dataset = OAIdataset(csv_file=train_Csv,
                           root_dir='C:/Users/Amir Kazemtarghi/Documents/INTERNSHIP/data/DatabaseB/',
...
y_score_sum = np.zeros((1780, 5))

tensorboardx(train_dataset, writer, A)


for nfold, (train_index, test_index) in enumerate(Groupkfold):
    ...
    model, train_loss, valid_loss = Training_dataset(data_loaders,
                                                     model,
                                                     patience,
                                                     Epoch,
                                                     namefold,
...
# y_score_ave = y_score_sum / 5
# values, indices = torch.max(y_score_ave, 0)
# y_pred = np.where(np.amax(y_score_ave), axis=1)
